...
class SequencePairClassifier(object):
    def __init__(self, seq_length=None, num_tasks=None, arch_fname=None, weights_fname=None,
                 ...
                 pool_width_first_lyr=5000, pool_width=25, L1=0, dropout=0.0,
...
##                self.model.add(Dropout(dropout))
            print(seq_length)
            print(pool_width_first_lyr)
            #self.model.add(MaxPooling2D(pool_size=(1, pool_width_first_lyr), input_shape=(1, 2, seq_length)))
            self.model.add(MaxPooling2D(pool_size=(1, 1000), input_shape=(1, 2, seq_length)))
            print("after maxpooling:", self.model.layers[-1].output_shape)
            #self.model.add(Convolution2D(
            #    nb_filter=25, nb_row=1,
            #    nb_col=1, activation='linear',
            #    init='he_normal', #input_shape=(1, conv_height, seq_length),
            #    W_regularizer=l1(L1), b_regularizer=l1(L1)))
            self.model.add(Activation('relu'))
            print("after relu:", self.model.layers[-1].output_shape)
            self.model.add(Dropout(dropout))
            print("after dropout:", self.model.layers[-1].output_shape)
            ...
            print("after flatten:", self.model.layers[-1].output_shape)
            ...
            print("after sigmoid classifier:", self.model.layers[-1].output_shape)
        else:
